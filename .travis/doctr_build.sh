...
if [ ! -z "$TRAVIS_TAG" ]; then
    ...
    echo "### [zip]"
    cp -r docs/_build/html docs/_build/pypkg_bintray_01-$TRAVIS_TAG
    cd docs/_build || exit 1
    zip -r "pypkg_bintray_01-$TRAVIS_TAG.zip" "pypkg_bintray_01-$TRAVIS_TAG"
    rm -rf "pypkg_bintray_01-$TRAVIS_TAG"
    cd ../../ || exit 1
    mv "docs/_build/pypkg_bintray_01-$TRAVIS_TAG.zip" docs/_build/artifacts/
    ...
    mv docs/_build/latex/*.pdf "docs/_build/artifacts/pypkg_bintray_01-$TRAVIS_TAG.pdf"

    echo "### [epub]"
    tox -e docs -- -b epub _build/epub
    mv docs/_build/epub/*.epub "docs/_build/artifacts/pypkg_bintray_01-$TRAVIS_TAG.epub"
    ...
    if [ ! -z "$TRAVIS" ]; then
        ...
        for filename in docs/_build/artifacts/*; do
            ...
            response=$(curl --upload-file "$filename" --user "$BINTRAY_USER:$BINTRAY_TOKEN" "$BINTRAY_UPLOAD")
        ...
        response=$(curl --request POST --user "$BINTRAY_USER:$BINTRAY_TOKEN" "$BINTRAY_RELEASE")
